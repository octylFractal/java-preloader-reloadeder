...
impl Error for JdkManagerError {}
...
impl JdkManager {
    ...
    pub fn get_installed_jdks(&self) -> ESResult<Vec<VersionKey>, JdkManagerError> {
        ...
        for ent in std::fs::read_dir(&*JDK_STORE_PATH)
            ...
            .attach_with(|| format!("Could not read JDK store at {:?}", *JDK_STORE_PATH))?
        {
            let ent = ent.change_context(JdkManagerError).attach_with(|| {
    ...
    pub fn get_full_version_from_path(
        ...
        let version = std::fs::read_to_string(&marker)
            .change_context(JdkManagerError)
            .attach_with(|| format!("Could not read JDK version from {:?}", marker))?;
        let version = JavaVersion::from_str(&version)
            .change_context(JdkManagerError)
            .attach_with(|| format!("Could not parse JDK version from {:?}", marker))?;
        Ok(Some(version))
    }
    ...
    pub fn download_jdk(
        ...
        let (list_info, info) = FOOJAY_API
            ...
            .change_context(JdkManagerError)
            .attach_with(|| format!("Could not get latest JDK package info for {}", jdk))?;
        ...
        let root = match Self::determine_jdk_root(unpack_dir.path())
            .change_context(JdkManagerError)
            .attach("Could not determine JDK root directory")
        {
            Ok(root) => root,
            Err(e) => {
                Self::cleanup_unpack_dir(unpack_dir);
        ...
        std::fs::write(marker_temp.path(), list_info.java_version.to_string())
            .change_context(JdkManagerError)
            .attach_with(|| format!("Could not write JDK version to {:?}", marker_temp.path()))?;
    ...
    fn download_jdk_to_file(
        ...
        let progress_bar = new_progress_bar(response.body().content_length()).with_message(
        ...
        match list_info.archive_type {
            ...
            ArchiveType::Zip => {
                ...
                for i in 0..archive.len() {
                    ...
                    let abs_unpack_target = unpack_dir.join(&rel_unpack_target);
                    if file.is_dir() {
                        writing_bar.set_message(
                            format!(
                                "Creating directory {}",
                        ...
                        std::fs::create_dir_all(&abs_unpack_target)
                            .change_context(JdkManagerError)
                            .attach_with(|| {
                                format!(
                                    "Could not create directory for extracted JDK at {:?}",
                                    abs_unpack_target
                                )
                            })?;
                        continue;
                    ...
                    if file.is_symlink() {
                        let target = {
                            ...
                            std::io::copy(&mut file, &mut target)
                                ...
                                .attach_with(|| {
                                    format!(
                                        "Could not read symlink target at {:?} from JDK archive",
                                        abs_unpack_target
                                    )
                                })?;
                            // Here is where I say "screw windows".
                            PathBuf::from(OsString::from_vec(target))
                        };
                        if !target.is_relative()
                            || target
                                .components()
                                .any(|it| matches!(it, Component::ParentDir))
                        {
                            warn!(
                                "Symlink {:?} may have unsafe target: {:?}, keeping it as JDK installs are generally trusted",
                                abs_unpack_target, target
                            );
                        }
                        writing_bar.set_message(
                            format!(
                        ...
                        std::os::unix::fs::symlink(&target, &abs_unpack_target)
                            .change_context(JdkManagerError)
                            .attach_with(|| {
                                format!("Could not create symlink at {:?}", abs_unpack_target)
                            })?;
                        continue;
                    }
                    if !file.is_file() {
                        warn!("Not extracting unknown entry: {}", file.name());
                        continue;
                    }
                    writing_bar.set_message(
                        format!(
                    ...
                    let mut extracted_file = std::fs::File::create(&abs_unpack_target)
                        .change_context(JdkManagerError)
                        .attach_with(|| {
                            format!("Could not create file at {:?}", abs_unpack_target)
                        })?;
    ...
    fn determine_jdk_root(unpack_dir: &Path) -> ESResult<PathBuf, JdkManagerError> {
        let entries = std::fs::read_dir(unpack_dir)
            .change_context(JdkManagerError)
            .attach_with(|| format!("Could not read JDK unpack directory at {:?}", unpack_dir))?
        ...
        if possible_home.join("bin/java").exists() {
            ...
            Err(Report::new(JdkManagerError).attach(format!(
